...
RegisterNetEvent("zaph:Anti-lag")
...
AddEventHandler("zaph:Anti-lag", function()
	if antilag == false then
			
			QBCore.Functions.Progressbar("antilag", "Egzoz Açılıyor", 800, false, false, {
				...
				antilag = true
				QBCore.Functions.Notify('egzoz patlatma aktif', "info")

			end, "fa-solid fa-burger")
	else
		
		QBCore.Functions.Progressbar("antilag", "Egzoz Kapatılıyor", 800, false, false, {
			disableMovement = false,
			disableCarMovement = false,
			disableMouse = false,
			disableCombat = true,
			...
			antilag = false
			QBCore.Functions.Notify('Egzoz patlatma inaktif', "info")
